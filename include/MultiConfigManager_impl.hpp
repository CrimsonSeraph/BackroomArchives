...
template <typename T>
inline std::optional<T> MultiConfigManager::get_with_name(const std::string &key_path,
...
template <typename T>
inline std::optional<T> MultiConfigManager::get_with_name_unsafe(const std::string &key_path,
                                                                 const std::string &key_name) const {
    if (key_path == "__priority") {
        return std::nullopt;
    }
...
template <typename T>
inline bool MultiConfigManager::set_with_priority(const std::string &key_path, const T &value, int target_priority) {
    std::lock_guard<std::mutex> lock(registry_mutex_);
    return set_with_priority_unsafe<T>(key_path, value, target_priority);
}

template <typename T>
inline bool MultiConfigManager::set_with_priority_unsafe(const std::string &key_path, const T &value,
                                                         int target_priority) {
...
template <typename T>
inline bool MultiConfigManager::set_with_name(const std::string &key_path, const T &value,
...
template <typename T>
inline bool MultiConfigManager::set_with_name_unsafe(const std::string &key_path, const T &value,
                                                     const std::string &key_name) {
    ...
    LOG_MODULE("MultiConfigManager", "set_with_name_unsafe", LOG_WARN, "未找到名称为 " << key_name << " 的配置管理器");
    return false;
}
