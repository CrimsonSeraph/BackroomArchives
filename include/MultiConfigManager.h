...
public:
    ...
    void register_config(const std::string &name, const std::string &file_path, bool auto_reload = false);
    ...
    /// @return 成功返回 true
    template <typename T>
    bool set_with_priority(const std::string &key_path, const T &value, int target_priority = -1);

    /// @brief 设置配置值到指定优先级的配置管理器（不加锁）
    template <typename T>
    bool set_with_priority_unsafe(const std::string &key_path, const T &value, int target_priority);
    ...
    /// @param key_name 配置名称
    template <typename T>
    bool set_with_name(const std::string &key_path, const T &value, const std::string &key_name);

    /// @brief 设置配置值到指定名称的配置管理器（不加锁）
    template <typename T>
    bool set_with_name_unsafe(const std::string &key_path, const T &value, const std::string &key_name);
