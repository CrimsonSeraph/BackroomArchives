...
template <typename T>
inline void AppConfig::set_value_with_priority(const std::string &key_path, const T &value, int target_priority) {
    std::lock_guard<std::mutex> lock(mutex_);
    set_value_with_priority_unsafe<T>(key_path, value, target_priority);
}

template <typename T>
inline void AppConfig::set_value_with_priority_unsafe(const std::string &key_path, const T &value,
                                                      int target_priority) {
    if (!multi_config_) {
...
template <typename T>
inline T AppConfig::get_value_with_name(const std::string &key_path, T default_value,
...
template <typename T>
inline T AppConfig::get_value_with_name_unsafe(const std::string &key_path, T default_value,
                                               const std::string &key_name) const {
    if (!multi_config_) {
        static bool logged = false;
        if (!logged) {
...
template <typename T>
inline void AppConfig::set_value_with_name(const std::string &key_path, const T &value, const std::string &key_name) {
    std::lock_guard<std::mutex> lock(mutex_);
    set_value_with_name_unsafe<T>(key_path, value, key_name);
}

template <typename T>
inline void AppConfig::set_value_with_name_unsafe(const std::string &key_path, const T &value,
