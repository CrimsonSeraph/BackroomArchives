...
JsConsoleBridge::JsConsoleBridge(QObject* parent)
    : QObject(parent) {
...
void JsConsoleBridge::log(const QString& level, const QString& message) {
    ...
    if (level_lower == "debug") log_level = LOG_DEBUG;
    else if (level_lower == "info") log_level = LOG_INFO;
    else if (level_lower == "warn") log_level = LOG_WARN;
    else if (level_lower == "error") log_level = LOG_ERROR;
    else log_level = LOG_DEBUG; // 默认 debug
