...
void MultiConfigManager::register_config(const std::string &name, const std::string &file_path, bool auto_reload) {
    ...
    for (const auto &[name, info] : config_registry_) {
        if (info.manager) {
            ...
            if (priorities.find(priority) != priorities.end()) {
                std::stringstream ss;
                ss << "优先级冲突: 配置 '" << name << "' 和 '" << priority_to_name[priority] << "' 有相同的优先级 "
                   << priority;
                error_msg = ss.str();
...
bool MultiConfigManager::load_all() {
    ...
    for (auto &[name, info] : config_registry_) {
        ...
        LOG_MODULE("MultiConfigManager", "load_all", LOG_DEBUG,
                   "配置加载成功，优先级=" << info.priority << " : " << name);
    ...
    cache_dirty_ = true;
    LOG_MODULE("MultiConfigManager", "load_all", LOG_INFO,
               "所有配置加载完成，全部成功=" << (all_success ? "是" : "否"));
    return all_success;
}
...
bool MultiConfigManager::save_all() {
    ...
    LOG_MODULE("MultiConfigManager", "save_all", LOG_INFO,
               "所有配置保存完成，全部成功=" << (all_success ? "是" : "否"));
    return all_success;
}
...
MultiConfigManager::~MultiConfigManager() { save_all(); }
    ...
    while (running_) {
        ...
        for (auto &[name, info] : config_registry_) {
            ...
            if (current_time > info.last_mod_time) {
                LOG_MODULE("MultiConfigManager", "file_watcher_loop", LOG_INFO, "检测到文件变化，重载配置: " << name);
                try {
                    if (info.manager->load()) {
                        auto priority_opt = info.manager->get<int>("__priority");
                        info.priority = priority_opt.value_or(0);
                        ...
                        cache_dirty_ = true;
                        LOG_MODULE("MultiConfigManager", "file_watcher_loop", LOG_INFO, "重载成功: " << name);
                    }
                    else {
                        LOG_MODULE("MultiConfigManager", "file_watcher_loop", LOG_ERROR, "重载失败: " << name);
                    }
                }
