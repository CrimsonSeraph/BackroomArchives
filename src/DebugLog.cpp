...
DebugLog &DebugLog::instance() {
    ...
    std::call_once(flag, [&]() {
        LogSink consoleSink;
        consoleSink.callback = [](const std::string &module, const std::string &method, LogLevel level,
            ...
            std::string formatted = padRight(tag1, TAG1_WIDTH) + " " + padRight(tag2, TAG2_WIDTH) + " " +
                                    padRight(tag3, TAG3_WIDTH) + ": " + message;
